...
<%
    ...
    string sqlstr = "SELECT distinct top 5 MessagesId,MessageName,MessageDate FROM xxfb  where sendto='全部'or sendto='" + Session["username"] + "' or sendto='查询' order by MessageDate desc ";
...
<a href="../check/Info/InfoRead.aspx?NewsId=<%=ds.Tables[0].Rows[i]["MessagesId"].ToString() %>" target="_parent">
<%=ds.Tables[0].Rows[i]["MessageName"].ToString()%></a></td>
